function my_mars_rois2img(roi_list, img_name, roi_space, flags)
% creates cluster or number labelled ROI image from ROIs

% Esto solo convierte los mat-s a nii, pero si Myriam ya me los pasa en nii
% no hay problema. 
...
% Create ROI_in.txt as ls in command line to write all files.
% Edit the .mat and create _0ut.txt with the nii files

% Remember to change the L_ and the R_ for lh. and rh.

% TO EXECUTE JUST PLAY THE FUNCTION AFTER EDITIING

ROIs = textread('ROI_in.txt', '%s');
filespath = textread('ROI_out.txt', '%s');
n = size(ROIs);
N = n(1);
SaveImagePath = ('./ROI_out');

mars.spacebase.fname = ('/export/home/glerma/glerma/matlab/spm8/canonical/single_subj_T1.nii');
...
if isempty(img_name)
  ...
  img_name = cell2mat(filespath(kk))
